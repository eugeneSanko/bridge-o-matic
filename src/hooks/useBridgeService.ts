...
export function useBridgeService() {
  const [lastPriceCheck, setLastPriceCheck] = useState<PriceResponse | null>(
    null
  );
  ...
  const fetchCurrencies = useCallback(async () => {
    try {
      console.log(
        "Fetching available currencies from FixedFloat API via Supabase Edge Function..."
      );
  ...
  const createOrder = useCallback(
    async (
      ...
      try {
        ...
        console.log("Using API Key:", API_CONFIG.API_KEY);
        console.log("Using API Secret Length:", API_CONFIG.API_SECRET.length);
        ...
        if (import.meta.env.DEV) {
          ...
          const mockOrderId = `FF-${Math.random()
            .toString(36)
            .substring(2, 10)
            .toUpperCase()}`;
          const mockOrderToken = `${Math.random()
            .toString(36)
            .substring(2, 30)}`;
