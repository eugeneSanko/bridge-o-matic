  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 p-1 bg-secondary/30 rounded-lg">
        <button 
          className={`py-2 px-3 sm:px-4 rounded text-sm sm:text-base ${value === 'fixed' ? 'bg-[#0FA0CE] text-white' : 'text-gray-400'} font-medium`}
          onClick={() => onChange('fixed')}
        >
          Fixed Flow 1%
        </button>
        <button 
          className={`py-2 px-3 sm:px-4 rounded text-sm sm:text-base ${value === 'float' ? 'bg-[#0FA0CE] text-white' : 'text-gray-400'} font-medium`}
