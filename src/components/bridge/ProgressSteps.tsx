...
export const ProgressSteps = ({ currentStatus = 'pending' }: ProgressStepsProps) => {
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={i} className={`text-center relative ${
              ...
              step.status === 'refunded' ? "text-yellow-500" :
              step.status === 'completed' ? "text-green-500" :
              step.active ? "text-[#0FA0CE]" : 
              step.completed ? "text-green-500" : 
              "text-gray-500"
            }`}>
              <div className="flex justify-center mb-3">
                <Icon className={`h-6 w-6 md:h-8 md:w-8 ${step.active && step.icon === Loader2 ? "animate-spin" : ""}`} />
              </div>
              <div className="text-xs md:text-sm font-medium">{step.label}</div>
              {step.status === 'failed' && <div className="text-xs text-red-500 mt-1">Transaction failed</div>}
              {step.status === 'refunded' && <div className="text-xs text-yellow-500 mt-1">Funds refunded</div>}
              {i < 3 && (
                <div className={`absolute top-4 left-[60%] w-[80%] h-[2px] ${activeStep > i ? "bg-green-700" : "bg-gray-700"}`} />
              )}
            </div>
          );
        })}
